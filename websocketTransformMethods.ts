...
type Commands = {
  ...
  JOIN: {} // 当用户进入时，告知后端
  ID: { userId: string; roomId: string; content: WebRTCIdentity } // 后端告知身份信息

  CAN_OFFER: {} //【主播接收】后端通知主播：可以发来OFFER了 // 仅限主播在第二个人加入后，收到
  CREATE_OFFER: { content: RTCSessionDescriptionInit; userId: ID; roomId: ID } //【主播发起】主播给后端offer信息
  RECEIVE_OFFER: { content: RTCSessionDescriptionInit; userId: ID; roomId: ID } //【观众接收】观众从后端收到主播的offer信息
  CREATE_ANSWER: { content: RTCSessionDescriptionInit; userId: ID; roomId: ID } //【观众发起】观众给后端answer信息
  RECEIVE_ANSWER: { content: RTCSessionDescriptionInit; userId: ID; roomId: ID } // 【主播接收】主播从后端收到观众的answer信息

  CANDIDATE: { content: RTCIceCandidate; userId: ID; roomId: ID }
}
/**
